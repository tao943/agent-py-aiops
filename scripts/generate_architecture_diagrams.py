...
SPECS = (
    DiagramSpec(
        ...
        8,
        {
            ...
            "right_modules": [
                ("数据与检索", "PostgreSQL · Milvus · BM25L / RRF"),
                ("真实外部系统", "Qwen · MCP · 腾讯 CLS"),
                ("本地运行底座", "JSON 配置 · Compose · 启动脚本"),
            ],
            "runtime": ["用户", "Vue 3", "HTTP / SSE", "FastAPI", "Chat / AIOps", "Qwen / MCP", "PostgreSQL / Milvus"],
    ...
    DiagramSpec(
        ...
        6,
        {
            ...
            "columns": [
                ...
                {"title": "数据与外部能力", "subtitle": "全部使用真实边界", "accent": "spec", "items": ["PostgreSQL 业务状态", "Milvus 知识向量", "Qwen 模型", "MCP / 腾讯 CLS"]},
    ...
    DiagramSpec(
        ...
        3,
        {
            ...
            "modules": ["Vue 3", "FastAPI", "API 契约", "RAG", "AIOps", "MCP", "PostgreSQL / Milvus", "文档"],
    ...
    DiagramSpec(
        ...
        1,
        {
            ...
            "branches": [
                ...
                {"title": "平台枝", "subtitle": "数据与运行", "items": ["FastAPI", "PostgreSQL", "Milvus", "本地启动"]},
    ...
    DiagramSpec(
        ...
        5,
        {
            ...
            "lanes": [
                ...
                {"title": "/knowledge 知识", "items": [("知识页面", "KnowledgeView"), "上传与切分预览", "持久索引任务", "PostgreSQL + Milvus"]},
    ...
    DiagramSpec(
        ...
        3,
        {
            ...
            "outcome": "Milvus 只存知识 chunk 与向量；PostgreSQL 保存文档、任务和业务状态",
        },
    ),
    DiagramSpec(
        25,
        ...
        {
            "center": "仓库级\nharness",
            "center_subtitle": "同时覆盖 Vue、FastAPI、LangGraph、PostgreSQL 与外部系统",
            "tag": "诊断过程可取消、可恢复、可追溯",
            "steps": [
                ("活跃告警", "Prometheus / Alertmanager"),
                ("持久任务", "PostgreSQL 后台任务"),
                ("规划", "先检索 SOP"),
                ("执行", "调用真实 SearchLog"),
                ("腾讯 CLS", "MCP 返回日志证据"),
                ("再规划", "依据已存证据进入报告"),
                ("报告", "结论关联证据链"),
                ("案例入库", "案例文档存 PostgreSQL · 分块写 Milvus")],
    ...
    DiagramSpec(
        ...
        "持久后台任务的接力系统",
        "harness 把 API、PostgreSQL、后台执行器、重试、恢复、SSE 与前端状态编码成一个整体",
        "pipeline",
        2,
        {
    ...
    DiagramSpec(
        ...
        "身份作用域从 API 进入数据访问层、PostgreSQL 与 Milvus，并覆盖所有业务能力",
        "boundary",
        5,
        {
            ...
            "columns": [
                ...
                {"title": "数据作用域", "subtitle": "PostgreSQL 与 Milvus 分工", "accent": "agent", "items": ["数据访问加 owner 条件", "Milvus 元数据带 user / tenant", "检索表达式权限过滤", "删除与覆盖同样受控"]},
    ...
    DiagramSpec(
        ...
        8,
        {
            ...
            "modules": [
                ...
                "MCP / CLS", "PostgreSQL / Milvus", "本地 JSON 配置", "Compose 基础设施", "全仓测试", "OpenSpec / 文档",
            ],
