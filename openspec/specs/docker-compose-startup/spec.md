...
项目 SHALL 在 `infra/compose.yaml` 中管理 PostgreSQL 16、Redis 7、etcd、MinIO、Milvus、Attu、Alertmanager 和 Nginx 本地基础设施。Nginx SHALL 作为访问宿主机 FastAPI 的统一开发入口；后端、前端和官方 CLS MCP Server SHALL 继续由本地启动器直接在宿主机运行，Compose SHALL NOT 构建或运行应用程序镜像。
