...
后端 SHALL 暴露一个类型化的 `/ready` 端点，该端点独立检查 PostgreSQL、Redis、Milvus、配置的 LLM 提供商以及本地 MCP 服务器，而不会暴露凭据。

#### Scenario: All required dependencies are ready
- **WHEN** PostgreSQL、Redis、Milvus、配置的 LLM 提供商以及配置的 MCP 服务器可用
...
- **THEN** 工作区标题 MUST 渲染连接状态，且响应 MUST 不依赖 PostgreSQL、Redis、Milvus、Qwen 或 MCP 的可用性。
...
后端 SHALL 暴露一个未经身份验证的类型化 `/config/check` 端点，该端点验证跟踪的配置并为 PostgreSQL、Redis、Milvus、Qwen 和 MCP 报告安全依赖项诊断信息。
