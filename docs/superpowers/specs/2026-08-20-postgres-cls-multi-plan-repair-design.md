...
4. 用离线测试证明 PostgreSQL Lock + CLS 的公开输入可以选择真正的 Multi-Agent 路由。
...
4. **场景路由**：使用 PostgreSQL Lock 场景的公开 hypotheses、发现工具和 CLS scope，完成计划规范化后，强制 Multi 选择 `runtime`、`log`，且不含 `insufficient_parallel_sources`。
5. **答案隔离**：测试数据不加载或向 Agent/Router 传入 `ground_truth.yaml`。
