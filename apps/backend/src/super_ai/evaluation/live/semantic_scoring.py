"""Deterministic evaluator-only matching for Live root-cause prose."""
...
def score_root_cause_semantics(
    decision: RootCauseDecision | None, oracle: ScenarioOracle
) -> RootCauseSemanticScore:
    """Score structured identity and private semantic requirements deterministically."""
    ...
    milestones = tuple(
        (
            ...
            if any(
                ...
                for step in decision.causal_chain
            )
            else 0,
        )
        for requirement in semantics.causal_milestones
    )
    return RootCauseSemanticScore(component, mechanism, trigger, milestones)


def _requirement_matches(
    text: str, requirement: SemanticRequirement, semantics: RootCauseSemantics
) -> bool:
